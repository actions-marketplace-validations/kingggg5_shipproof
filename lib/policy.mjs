...
export function validatePolicy(rawPolicy) {
  ...
  rejectUnknown(scan, new Set(["path", "exclude", "max_file_bytes", "profile"]), "scan");
  ...
  const profile = scan.profile || "auto";
  if (!new Set(["auto", "application", "library"]).has(profile)) {
    throw new Error("scan.profile must be auto, application, or library");
  }

  const security = requireObject(policy.security || {}, "security");
  rejectUnknown(security, new Set(["fail_on", "fail_on_incomplete", "block_min_proof"]), "security");
  const failOn = security.fail_on || "high";
  if (!SEVERITIES.has(failOn)) throw new Error("security.fail_on has an unsupported severity");
  if (security.fail_on_incomplete !== undefined && typeof security.fail_on_incomplete !== "boolean") {
    throw new Error("security.fail_on_incomplete must be a boolean");
  }
  const blockMinProof = security.block_min_proof || "L1";
  if (!new Set(["L0", "L1", "L2"]).has(blockMinProof)) {
    throw new Error("security.block_min_proof must be L0, L1, or L2");
  }
  ...
  return {
    ...
    security: {
      fail_on: failOn,
      ...(security.fail_on_incomplete === undefined ? {} : { fail_on_incomplete: security.fail_on_incomplete }),
      ...(security.block_min_proof === undefined ? {} : { block_min_proof: blockMinProof }),
    },
    performance,
    capacity,
  };
...
export function buildPolicyGates(repositoryRoot, policy) {
  ...
  if (policy.security.fail_on_incomplete) scanArguments.push("--fail-on-incomplete");
  if (policy.security.block_min_proof) scanArguments.push("--block-min-proof", policy.security.block_min_proof);
  if (policy.scan.profile) scanArguments.push("--scan-profile", policy.scan.profile);
