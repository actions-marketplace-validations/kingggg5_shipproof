...
- Default scans now require L1 proof (AST/taint/structural) or an allowlisted L0 finding such as a high-confidence secret or `DEBUG = True` before they `BLOCK`. Remaining high L0 findings still appear, but the verdict is `REVIEW`. Override with `--block-min-proof` / `security.block_min_proof`; `shipproof check` will not honor a floor below L1. Auto library mode downranks application-oriented rules when no application entrypoint is visible; set `scan.profile` to `application` or `library` to pin it. Nested `wsgi.py` / `asgi.py` modules no longer force application profile, so WSGI/ASGI libraries stay in library mode. SP061 is advisory and no longer changes the verdict. Terminal and Markdown collapse extra copies of the same rule in one file; JSON and SARIF keep every finding.
- Head-to-head labels are schema 3: fixture corpora now have sink-line labels, CI gates line-level precision/recall at 0.95 on the ShipProof self-leg, and Linux Semgrep comparison is an opt-in workflow input that uses only the original ruleset in `benchmarks/semgrep-comparison/`. OWASP Benchmark Java/Python stay unbundled after a GPL-2.0/GPL-3.0 license review.
- Add a revision-pinned clean-corpus evaluator (`scripts/eval-clean-corpus.py`) and a finding-label store (`benchmarks/labels/`, `schemas/finding-label.schema.json`) so precision is scored only on reviewed rows. Context gates now suppress the common clean-library false positives (SHA1 without password context, `fnmatch.filter`, `while True` plus `recv`, MIME-only SSE strings, framework SSE implementations, cache pickle loads, docstring DSNs, `exec(compile)` config loaders, debug REPLs, and inflate `.pipe()` with nearby destroy/unpipe). License review accepts `dist-info/licenses/` and license classifiers so wheels such as Jinja2 are scannable. Clean-corpus coverage expands to at least 30 pinned packages across Python, JavaScript, Go, and Rust. Weekly and release workflows enforce `benchmarks/precision-thresholds.json` through `scripts/check-precision-trend.py`. The ten remaining medium gate leftovers on the original 15-package measurement are labeled `false_positive` after source review.
- Add opt-in maintainer lanes that stay off the default scanner path: bounded ZIP/Office inspection (`--inspect-archives`, virtual `archive.zip!/inner` paths, no extract/execute), an advisory shadow harness for research prototypes SP5301/SP5951/SP6309, a Skill/MCP capability inventory, and versioned external-evidence import (`shipproof gate evidence --import`) that keeps original rule IDs and exits `2` on malformed envelopes. The agent red-team lab is a deterministic local fixture only.
- Close the release-blocker trust-boundary audit: Python, Git, Go, and Rust probes now resolve only trusted absolute host executables; the TypeScript adapter requires explicit consent plus a repository-contained regular compiler; scan coverage, parser/line/finding/baseline/JSON inputs, report writes, policy gates, and terminal output fail closed or stay bounded across CLI, Action, pre-commit, and MCP channels. Direct scans also scope runtime discovery to the selected target path. The approved unpacked package budget is 1.86 MB to include the 0.11.0 release notes with the shared resolver and precision-plan measurement assets without weakening the exact file allowlist.

- Scope precision updates for `SP210`, `SP220`, `SP583`, `SP597`, and `SP599` around workflow run context, verified Git tracking, explicit BullMQ stalled-check opt-out, same-component independent fetches, and JSON-source non-null dereferences. The MCP verdict TTL cache is disabled; the legacy `SHIPPROOF_MCP_CACHE_MS` environment variable remains accepted and validated but is ignored.

- Fix the critical SP631 false positive where `ledger` satisfied the substring `edge`: require a literal module-level Edge runtime declaration and a native runtime import, reject comment/string/type-import look-alikes, preserve positive contracts, and document unsupported syntax without claiming generic Node.js serverless incompatibility. Human-readable scan verdicts now also respect `--include-tests`.
- Add a selected-source coverage ledger. Unreadable files/directories, oversized source files, and uninspected containers make coverage incomplete; intentional scope exclusions remain separate, and pruned trees are not inventoried. JSON, SARIF, trace, human-readable output, and policy checks retain incomplete status; a would-be pass becomes `CONDITIONAL`. The optional completeness gate exits `1`, including with `--fail-on none` or autofix/dry-run. `.shipproof-research-cache/` joins the default skip directories.
- Recheck P0/P1 coverage boundaries: apply changed-only scope and baseline/exclude decisions before omission counts, report directory-walk failures, count only successfully inspected files, reset counters on parallel fallback, and exclude unreadable files from cross-file candidates. Bound source and framework-manifest reads, reject non-regular files/final symlinks, prune Windows reparse directories, and inspect databases through a fixed header only.
- Expose the completeness gate through repository policy (`security.fail_on_incomplete`), GitHub Action (`fail-on-incomplete`), and MCP (`fail_on_incomplete`). Repository scans, the Action, and MCP now fail closed by default; explicit Action/MCP `false` values translate to the visible `--allow-incomplete` exploratory override. Inputs reject boolean coercion. Action summaries show incomplete coverage, and strict MCP calls include the decision trace so callers can see the gate failure without a process exit code.
- Upgrade baselines to auditable suppression (format version 2): fingerprint objects and human-authored glob rules (`id`, `path`, `evidence`) require non-empty reasons; legacy string fingerprints keep loading. `--baseline-out` defaults to an explicit review-required reason, preserves rule matchers without empty fields, and respects loader limits. Reject duplicate/unknown fields, control characters, more than 256 rules or 10,000 fingerprints, and input/output above 2,000,000 bytes. `--show-suppressed` supports JSON, Markdown, terminal and SARIF (external suppressions); incompatible modes fail with exit `2`. The baseline itself is excluded from scanning, and a major scanner-version mismatch warns. Finding fingerprints are not whole-file digests or signed approvals.
- Treat authored eval-dataset files (`evals.json`, `dataset.jsonl`, and siblings under `evals/` or `eval/` directories) as test scope so prompts, expected outputs, and ground-truth strings no longer trigger non-secret rules in AI repositories; secrets inside them are still reported, at test scope. `.jsonl` is scanned as text so label stores and eval datasets are not omitted as binary coverage holes.

- Scope the Skill-security rules (`SP096`–`SP100`) to declared Skill packages, including changed-only and parallel scans, so ordinary README prose and unrelated source files do not become Skill findings.
- Make `SP099` and `SP100` capability findings truthful and non-blocking by reporting observable credential access or webhook POST behavior at medium severity without claiming proven harvesting or exfiltration; ordinary model-provider API calls remain silent.
- Correct `SP282` for Ollama's model-name semantics: bare names, namespaces, `library/*`, and the default `registry.ollama.ai` stay silent, while explicit non-default registries remain reportable. Add curated positive, negative, and adversarial contracts for the boundary.
- Describe the AI-framework rules as source-level security-posture checks rather than deployed-CVE identification, and record an evidence-gated roadmap for Skill, MCP, and agent-workflow security without adding network access to the default scan.
