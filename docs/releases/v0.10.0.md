...
- Current documentation is scanner-derived at 638 executable rules, and a structure contract fails if the authoritative English, Thai, rule, research, plan, or release claims drift again.
...
- All 638 executable scanner rules now have machine-readable assurance coverage for positive, negative, adversarial, CWE/control, remediation, and false-positive boundaries.
...
- Repository scans, the pre-commit hook, the GitHub Action, and MCP now fail closed when coverage is incomplete by default. `--allow-incomplete` is an explicit exploratory override and is never used by `check`.

## Evidence boundary

The three promotion prototypes change the 638-rule public catalog through the non-blocking SP666-SP668 wave. Their fixtures show that a detector shape is testable; representative repository precision remains pending.
