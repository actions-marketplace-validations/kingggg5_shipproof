...
export function buildScanArguments({
  ...
  if (typeof fail_on_incomplete !== "boolean") {
    throw new Error("fail_on_incomplete must be a boolean");
  }
  const argumentsList = [
    path,
    "--format",
  ...
  // MCP has no process exit code for callers: expose the requested gate via
  // the existing decision-trace contract as well as the completeness ledger.
  if (fail_on_incomplete) argumentsList.push("--fail-on-incomplete", "--trace");
  else argumentsList.push("--allow-incomplete");
...
function runPythonProcess(command, argumentsList, { abortSignal, stdin, rootPath = process.cwd() } = {}) {
  const { timeoutMs } = mcpSettings();
  const python = detectPythonRuntime({ rootPath });
  if (!python) {
...
export function buildMcpOutputSchemas(z) {
  ...
  const verdict = z.enum(["PASS", "PASS_WITH_EVIDENCE", "CONDITIONAL", "WARN", "REVIEW", "BLOCK"]);
  const common = {
...
export async function startMcpServer() {
  ...
  if (activeMcpSettings.cacheMs > 0) {
    // Keep validating the legacy setting for 0.x configuration compatibility,
    // but never reuse a verdict without fresh source evidence. Runtime discovery
    // remains cached separately; it contains no repository findings.
    console.error("shipproof: SHIPPROOF_MCP_CACHE_MS is deprecated and ignored; scans always read current files");
  }
  let McpServer;
  ...
  server.registerTool(
    "shipproof_scan",
    {
      ...
      inputSchema: {
        ...
        fail_on_incomplete: z.boolean().default(true),
  ...
  server.registerTool(
    ...
    async (input, extra) => {
      try {
        ...
        return toolSuccess(await runPythonJson("capacity", argumentsList, extra.signal, { rootPath: repositoryRoot }));
      } catch (error) {
        return toolFailure(error);
      }
    },
    ...
    async ({ code, filename }, extra) => {
      try {
        ...
        const report = await runPythonJson(
          ...
          extra.signal,
          { stdin: code, rootPath: repositoryRoot },
        );
        return toolSuccess(report);
      } catch (error) {
        return toolFailure(error);
