...
const SECURITY_FLOOR_ORDER = { critical: 0, high: 1, medium: 2, low: 3, none: 99 };
const CHECK_MAX_FILE_BYTES = 10_000_000;
// `scan` delegates option validation to scan_repo.py, but the launcher still
// needs to identify its target before probing Python. Keep every option that
// consumes a following value here so a snippet/config value cannot be treated
// as the repository path (and so runtime discovery is scoped to that target).
const SCAN_OPTIONS_WITH_VALUES = [
  "--format",
  "--output",
  "--baseline",
  "--baseline-out",
...
function resolveScanRoot(commandArguments) {
  return resolve(readPositionalArgument(commandArguments, ".", SCAN_OPTIONS_WITH_VALUES));
}
...
function runDoctorCommand(rootPath, useJsonOutput) {
  ...
  const pythonRuntime = detectPythonRuntime({ rootPath: repositoryRoot });
...
function runPythonCommand(
  ...
  const pythonRuntime = detectPythonRuntime({ rootPath });
  if (!pythonRuntime) {
  ...
  const processResult = spawnSync(pythonRuntime.command, [
    ...
    // Keep the caller's argument semantics (relative paths are resolved from
    // the original launcher cwd) while using rootPath solely as the trust
    // boundary for runtime discovery. Commands that intentionally operate on
    // their root can pass the same value for both arguments.
...
function gitOutput(repositoryRoot, argumentsList, { allowMissing = false } = {}) {
  const git = resolveTrustedExecutable("git", { root: repositoryRoot });
  if (!git) throw new Error("git command is unavailable outside the repository workspace");
...
function resolveGateLimits(environment = process.env) {
  return {
    timeoutMs: resolvePositiveIntEnv("SHIPPROOF_GATE_TIMEOUT_MS", 120_000, 1_000, environment),
...
function runCheckCommand(rootPath, commandArguments) {
  ...
  // Repository policy is changeable input. A pull request must not be able to
  // make `check` green by lowering the threshold, narrowing the path, or
  // excluding the vulnerable file. Stricter thresholds remain supported, but
  // the production check always scans the full root and fails on omissions.
  const configuredFailOn = policy.security.fail_on;
  const effectiveFailOn = SECURITY_FLOOR_ORDER[configuredFailOn] > SECURITY_FLOOR_ORDER.high
    ? "high"
    : configuredFailOn;
  const proofRank = { L0: 0, L1: 1, L2: 2 };
  const configuredProof = policy.security.block_min_proof || "L1";
  const effectiveProof = proofRank[configuredProof] < proofRank.L1 ? "L1" : configuredProof;
  const hardenedPolicy = {
    ...policy,
    scan: {
      ...policy.scan,
      path: ".",
      exclude: [],
      // Keep enough headroom for reviewed source/manifest files, while still
      // bounding hostile inputs. Files above this cap fail the completeness gate.
      max_file_bytes: CHECK_MAX_FILE_BYTES,
    },
    security: {
      ...policy.security,
      fail_on: effectiveFailOn,
      fail_on_incomplete: true,
      block_min_proof: effectiveProof,
    },
  };
  const gates = buildPolicyGates(repositoryRoot, hardenedPolicy).map((gate) => {
    const scriptPath = PYTHON_COMMANDS.get(gate.command);
    if (!scriptPath) throw new Error(`unsupported policy gate: ${gate.command}`);
    const result = runPythonJsonCommand(scriptPath, gate.argumentsList, spawnSync, repositoryRoot);
    return {
      name: gate.name,
  ...
  const incomplete = gates.some((gate) => gate.evidence.summary?.completeness?.is_complete === false);
  const report = {
    schema_version: "1.0",
    ...
    verdict: passed
      ? (incomplete ? "CONDITIONAL" : gates.some((gate) => gate.verdict === "REVIEW") ? "REVIEW" : "PASS_WITH_EVIDENCE")
      : "BLOCK",
    passed,
    gates,
    limitations: [
      "A passing policy proves only the declared deterministic gates; deployment still requires human review.",
      "Repository-authored policy is untrusted input; check enforces a high security floor, full-root scan, and fail-closed coverage.",
      ...(incomplete ? ["Scan coverage is incomplete; review the scan completeness ledger. The production check is fail-closed for omitted content."] : []),
    ],
  };
...
export function runCli(argumentsList) {
  ...
  try {
    ...
    if (commandName === "scan") {
      // Runtime discovery and the child process cwd must match the scanner's
      // actual target. Otherwise a scan of a sibling/absolute path could leave
      // that path outside the repository-shadowing trust boundary.
      return runPythonCommand(
        PYTHON_COMMANDS.get("scan"),
        commandArguments,
        resolveScanRoot(commandArguments),
        process.cwd(),
      );
    }
